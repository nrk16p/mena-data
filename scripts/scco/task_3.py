...
def load_data(ldf_path, driver_path, zone_path):
    return pd.read_excel(ldf_path), pd.read_excel(driver_path), pd.read_excel(zone_path)


def preprocess_driver_data(df_driver):
    df_driver["ทะเบียน"] = df_driver["ทะเบียน"].str.replace("สบ.", "", regex=True)
    return df_driver[["ทะเบียน","สถานะ","เบอร์รถ","รหัส.1","ชื่อ.1"]]
...
def reorder_columns(df_ldf):
    required_columns = [
        ...
        "ประเภทการวิ่ง","ประเภทการขนส่งขากลับ","ประเภทการขนส่ง","Service Parameter A",
        "Service Parameter B","แพล้นท์","Base Plant","วันที่","วันที่ครบกำหนด",
        ...
        "ทะเบียนหัว","ทะเบียนหาง","เวลาออกเดินทาง","นน ต้นทาง","นน ปลายทาง",
